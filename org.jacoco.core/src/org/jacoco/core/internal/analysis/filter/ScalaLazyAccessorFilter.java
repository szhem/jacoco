...
public class ScalaLazyAccessorFilter extends ScalaFilter {
	...
	private static class LazyAccessorMatcher extends AbstractMatcher {
		void ignoreMatches(final MethodNode methodNode,
			...
			MethodInsnNode lzyCompute = (MethodInsnNode) cursor;
			if (lzyCompute == null
					|| !lzyCompute.desc.equals(methodNode.desc)
					|| !lzyCompute.name.equals(methodNode.name + LAZY_SUFFIX)) {
				return;
			}

			// find jump to the method return point
			nextIs(Opcodes.GOTO);

			// find another branch ("else" statement) whether the comparison of
			// the bit against 0 fails indicating, that the lazy field has
			// already been initialized
			nextIsVar(Opcodes.ALOAD, "this");

			// find get field instruction that loads already initialized field
			nextIs(Opcodes.GETFIELD);
			FieldInsnNode lzyField = (FieldInsnNode) cursor;
			if (lzyField == null
					|| !lzyField.name.equals(methodNode.name)
					|| !("()" + lzyField.desc).equals(methodNode.desc)) {
				return;
			}
